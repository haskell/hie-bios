...
callableToProcess (Command shellCommand) files = do
  ...
  let updated_env = case files of
        []         -> Nothing
        xs@(x : _) ->
          Just $
            [ (hie_bios_multi_arg, "\x1f" `intercalate` xs)
            , (hie_bios_arg, x)
            ] ++
            old_env
  return $ (shell shellCommand){env = updated_env}
callableToProcess (Program path) files = do
  ...
  let updated_env = case files of
        [] -> Nothing
        _  ->
          Just $ (hie_bios_multi_arg, "\x1f" `intercalate` files) : old_env
  return $ (proc canon_path (take 1 files)){env = updated_env}
